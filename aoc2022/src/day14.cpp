...
int drop(vector<vector<bool>>& tiles, pii coord, int minX, int maxX) {
	if (coord.first == minX || coord.first == maxX) {
		return coord.first;
	}
	if (!tiles[coord.first][coord.second+1]) {
		return drop(tiles, {coord.first, coord.second+1}, minX, maxX);
	}
	else if (!tiles[coord.first-1][coord.second+1]) {
		return drop(tiles, {coord.first-1, coord.second+1}, minX, maxX);
	}
	else if (!tiles[coord.first+1][coord.second+1]) {
		return drop(tiles, {coord.first+1, coord.second+1}, minX, maxX);
	}
...
int main() {
	...
	int prev = INT_MIN;
	while (prev == INT_MIN) {
		prev = drop(tiles, {500, 0}, minX, maxX);
		res++;
	}

	cout << res-1 << " " << prev << endl;
}
